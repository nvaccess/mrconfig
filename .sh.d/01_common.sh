...
registerAddon () {
    ...
    tmpName=$(mktemp -d)
    if git clone $(getGithubURL $addonName) ${tmpName}; then
        rm -rf ${tmpName}
        echo "[addons/${addonName}]" >10_${addonName}
        echo "checkout = git clone \$(getGithubURL) \$MR_REPO" >>10_${addonName}
        ...
        echo "unable to clone the ${addonName} repo from github nvdaaddons, please make sure the name is correct."
        exit 1
    fi
}
